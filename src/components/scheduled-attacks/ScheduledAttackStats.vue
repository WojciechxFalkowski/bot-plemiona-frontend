<template>
  <div class="border border-gray-200 rounded-lg">
    <div class="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 p-4">
      <button
        type="button"
        class="text-center focus:outline-none cursor-pointer py-2 hover:bg-gray-50 rounded-md transition-colors"
        :class="selectedFilter === 'all' ? 'bg-gray-100' : ''"
        @click="handleSelectFilter('all')"
        aria-label="Pokaż wszystkie ataki"
      >
        <div class="text-2xl font-bold text-gray-900">{{ totalCount }}</div>
        <div class="text-sm text-gray-600">Łącznie</div>
      </button>
      ...
      >
        ...
        <div class="text-sm text-gray-600">Zaplanowane</div>
      </button>
      ...
      >
        ...
        <div class="text-sm text-gray-600">Wykonywane</div>
      </button>
      ...
      >
        ...
        <div class="text-sm text-gray-600">Nieudane</div>
      </button>
      ...
      >
        <div class="text-2xl font-bold text-gray-600">{{ cancelledCount }}</div>
      ...
      >
        ...
        <div class="text-sm text-gray-600">Wygasłe</div>
      </button>
    </div>
  </div>
</template>
