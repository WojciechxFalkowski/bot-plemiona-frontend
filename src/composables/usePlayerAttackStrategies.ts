...
const convertFormDataToDto = (formData: AttackStrategyFormData & { is_active?: boolean }, serverId: number, villageId: string): CreateAttackStrategyDto => {
  ...
  Object.entries(formData).forEach(([key, value]) => {
    ...
    if (!isNaN(numValue)) {
      (dto as any)[key] = numValue; // Zawsze dodaj, nawet jeśli 0
    } else {
      (dto as any)[key] = 0; // Domyślnie 0 jeśli puste
    }
  });
...
const fetchStrategies = async (serverId?: number) => {
  try {
    ...
    toast.add({
      title: 'Błąd',
      description: 'Nie udało się pobrać strategii ataku',
      color: 'red'
    });
  } finally {
...
const createStrategy = async (formData: AttackStrategyFormData, serverId: number, villageId: string) => {
  try {
    ...
    toast.add({
      title: 'Sukces',
      description: 'Strategia ataku została utworzona',
      color: 'green'
    });
...
const updateStrategy = async (id: number, formData: AttackStrategyFormData & { is_active?: boolean }, serverId: number, villageId: string) => {
  try {
    ...
    const dto = convertFormDataToDto(formData, serverId, villageId);
    // Usuń serverId i villageId z DTO, bo backend go nie oczekuje w update
...
const getStrategiesByVillage = async (serverId: number, villageId: string) => {
  try {
    const response = await fetch(`${API_BASE}/village/${serverId}/${villageId}`);
    if (!response.ok) {
      throw new Error('Failed to fetch village strategies');
    }
    return await response.json();
  } catch (error) {
    console.error('Error fetching village strategies:', error);
    return null;
  }
};
